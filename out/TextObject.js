...
export class TextObject extends DrawnObjectBase {
    ...
    _recalcSize(ctx) {
        ...
        if (ctx) {
            ...
            // adding padding, get height from bounding box ascent / descent
            this.size = { w: textMetrics.width + this.padding.w, h: textMetrics.actualBoundingBoxAscent + textMetrics.actualBoundingBoxDescent + this.padding.h };
        }
    ...
    _drawSelfOnly(ctx) {
        ...
        try {
            ...
            ctx.fillStyle = clr;
            const textMetrics = ctx.measureText(this.text);
            const baseline = this.y + this.padding.h + textMetrics.actualBoundingBoxAscent;
            // Draw the text at the object's position
            if (this.renderType === 'fill') {
                ctx.fillStyle = clr; // Set fill color
                ctx.fillText(this.text, this.x + this.padding.w, baseline); // Draw filled text
            }
            else {
                ctx.strokeStyle = clr; // Set stroke color
                ctx.strokeText(this.text, this.x + this.padding.w, baseline); // Draw stroked text
            }
        }
        finally {
