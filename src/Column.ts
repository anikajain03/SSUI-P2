...
export class Column extends Group { 
    ...
    protected override _doLocalSizing() : void {
        ...
        for (let child of this.children) {
            if (!(child instanceof Spring)) {
                heightSum += child.naturalH; // Sum natural heights
                maxWidth = Math.max(maxWidth, child.naturalW); // Find max width
            }
        }
        // Set width configurations
        this.wConfig = SizeConfig.fixed(maxWidth);
        // Set height configurations
        this.hConfig = SizeConfig.fixed(heightSum);
    }
    ...
    protected _measureChildren() : [number, number, number] {
        ...
        // iterate through children
        for (let child of this.children) {
            // add to spring count if child is an instance of Spring
            if (child instanceof Spring) {
                numSprings++;
            }
            else {
                // add to natural sum
                natSum += this.naturalH;
                // calculate available compression (nat - min)
                availCompr += (this.naturalH - this.minH);
            }
        }
    ...
    protected _expandChildSprings(excess : number, numSprings : number) : void {
        ...
        // no springs -- do nothing
        if (numSprings === 0) return;

        // calculate new spring space
        const newSpringSpace = excess / numSprings;

        // iterate through children
        for (let child of this.children) {
            if (child instanceof Spring) {
                // set height of child to new spring space
                child.h = newSpringSpace;
            }
        }
    }
        ...
        availCompr : number)  // total compressability across all children
    {
        ...
        for (let child of this.children) {
            //=== YOUR CODE HERE ===
            // spring has zero natural size, already assumed in shortfall
            if (!(child instanceof Spring)) {
                // determine amount of compression
                const compressAmount = ((child.naturalH - child.minH) / availCompr) * shortfall;
                child.h = Math.max(child.h - compressAmount, child.minH); // Set new height
            }
        }
}
    ...
    protected override _completeLocalLayout() : void { 
        ...
        for (let child of this.children) {
            ...
            if (this.wJustification === 'left') {
                ...
                child.x = (this.w - child.w) / 2; // Center align
            }
