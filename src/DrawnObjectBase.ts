...
export class DrawnObjectBase {  
   ...
   //-------------------------------------------------------------------
    ...
    public set w(v : number) {
        //=== YOUR CODE HERE ===
        v = SizeConfig.withinConfig(v, this.wConfig); // clamping
        if (!(v === this._w)) {
            this._w = v;
            this.damageAll();
        }
    ...
    public set h(v : number) {
        //=== YOUR CODE HERE ===
        v = SizeConfig.withinConfig(v, this.hConfig); // clamping
        if (!(v === this._h)) {
            this._h = v;
            this.damageAll();
        }
    ...
    protected _startChildDraw(childIndx : number, ctx: DrawContext) {
        ...
        ctx.translate(child.x, child.y); // move to child's coordinate system
        this.applyClip(ctx, 0, 0, child.w, child.h); // clip to child's bounding box
    }
    ...
    public damageArea(xv: number, yv : number, wv : number, hv : number) : void {
        ...
        // notify parent of damage and new params
        if (this.parent) {
                               ...
                               wv : number, hv: number) : void 
    {
        ...
        const childAbsX = this.x + xInChildCoords;
        const childAbsY = this.y + yInChildCoords;

        // report damage
        this.damageArea(childAbsX, childAbsY, wv, hv);
    }
