...
export class Row extends Group { 
    ...
    protected override _doLocalSizing() : void {
        ...
        for (let child of this.children) {
            // update total widths
            totalWidthMin += child.wConfig.min;
            totalWidthNat += child.wConfig.nat;
            totalWidthMax += child.wConfig.max;
    ...
    protected _measureChildren() : [number, number, number] {
        ...
        for (let child of this.children) {
            // update spring count if child is instance of Spring
            if (child instanceof Spring) {
                numSprings++;
            } 
            else {
                // accummulate nat sum and available compressibility
                natSum += child.wConfig.nat;
                availCompr += (child.wConfig.nat - child.wConfig.min); // nat - min
            }
        }
    ...
    protected _expandChildSprings(excess : number, numSprings : number) : void {
        //=== YOUR CODE HERE ===
        // only proceed if there are springs
        if (numSprings === 0) return;

        const spacePerSpring = excess / numSprings;

        // allocate space to each spring
        for (let child of this.children) {
            if (child instanceof Spring) {
                // increase width
        ...
        availCompr : number)  // total compressability across all children
    {
        ...
        for (let child of this.children) {
            //=== YOUR CODE HERE ===
            // check if child is not spring
            if (!(child instanceof Spring)) {
                // determine fraction of available compressibility
                const childCompressibility = child.wConfig.nat - child.wConfig.min;
                const fracAvailCompr = childCompressibility / availCompr;

                // determine how much to compress for this child
                const compressionAmount = shortfall * fracAvailCompr;
                child.w = Math.max(child.wConfig.min, child.wConfig.nat - compressionAmount);
            }
        }
}
    ...
    protected override _completeLocalLayout() : void { 
        ...
        //=== YOUR CODE HERE ===
        let yOffset: number = 0; // Start at the top

        if (this.hJustification === 'center') {
            yOffset = (this.h - hMax) / 2; // Center vertically
        }
        else if (this.hJustification === 'bottom') {
            yOffset = this.h - hMax; // Align to the bottom
        }

        // Set the vertical position for each child based on the justification
        for (let child of this.children) {
            child.y = yOffset;
        }
    }
