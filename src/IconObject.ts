...
export class IconObject extends DrawnObjectBase {
    ...
    protected _resize() {
        ...
        // do nothing if image is not loaded
        if (!this._image || !this._image.loaded) { return; }

        // set width and height to match image size
        if (!this._resizesImage && this._image.canvasImage) {
    ...
    protected override _drawSelfOnly(ctx: DrawContext): void {
        ...
        if (this.resizesImage) {
            ...
            //=== YOUR CODE HERE ===
            ctx.drawImage(this.image.canvasImage, 0, 0, this.naturalW, this.naturalH);
        }
    }
